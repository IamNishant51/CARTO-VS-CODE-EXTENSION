...
---`);
    ...
    if (this.options.includeExecutiveSummary) tocEntries.push('- [Overview](#overview)');
    if (this.options.includeTechStack) tocEntries.push('- [Tech Stack](#tech-stack)');
    tocEntries.push('- [Project Statistics](#project-statistics)');
    if (this.options.includeTree) tocEntries.push('- [Directory Structure](#directory-structure)');
    ...
    if (this.options.includeFileContents) tocEntries.push('- [Source Files](#source-files)');
  ...
  private generateOverview(bundlerResult: BundlerResult, techStack?: TechStackInfo): string {
...
The following sensitive files were detected in this codebase. Review carefully before sharing this bundle with external services.\n\n`;
  ...
  private generateTechStack(techStack?: TechStackInfo): string {
    ...
    const rows: string[] = [];
    if (techStack.ecosystem)      rows.push(`| Ecosystem          | ${techStack.ecosystem} |`);
    if (techStack.language)       rows.push(`| Primary Language   | ${techStack.language} |`);
    if (techStack.runtimeVersion) rows.push(`| Runtime            | ${techStack.runtimeVersion} |`);
    if (techStack.name)           rows.push(`| Package Name       | \`${techStack.name}\` |`);
    if (techStack.version)        rows.push(`| Version            | ${techStack.version} |`);
  ...
  private generateProjectStats(summary: BundlerResult['summary']): string {
...
\`\`\``;
  ...
  private generateFileContents(files: BundledFile[]): string {
    ...
    const byDir = new Map<string, BundledFile[]>();
  ...
  private formatAsTwoColumns(items: string[]): string {
    if (items.length <= 5) {
